  ...
  const fetchAnalytics = async () => {
    try {
      const [growth, roles, trends, uploaders] = await Promise.all([
        fetch(`${process.env.REACT_APP_API_URL}/api/admin/user-growth`, {
          headers: { Authorization: `Bearer ${ token }` },
        }).then((r) => r.json()),
        fetch(`${ process.env.REACT_APP_API_URL } / api / admin / role - distribution`, {
          headers: { Authorization: `Bearer ${token}` },
        }).then((r) => r.json()),
      fetch(`${process.env.REACT_APP_API_URL}/api/admin/file-trends`, {
          headers: { Authorization: `Bearer ${ token }` },
        }).then((r) => r.json()),
        fetch(`${ process.env.REACT_APP_API_URL } / api / admin / top - uploaders`, {
          headers: { Authorization: `Bearer ${token}` },
        }).then((r) => r.json()),
      ]);
